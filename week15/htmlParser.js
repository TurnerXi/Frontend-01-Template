...
module.exports = class HtmlParser {
  ...
  parse(html) {
    ...
    for (const c of html) {
      ...
      if (
        this.stack[this.stack.length - 1].tagName === 'script' &&
        this.state === this.parseData
      ) {
        this.state = this.parseScriptData;
      }
    }
  ...
  parseScriptData(c) {
    if (c !== '<') {
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    } else {
      return this.parseScriptDataLessThan;
    }
  }

  parseScriptDataLessThan(c) {
    if (c === '/') {
      return this.parseScriptDataEndTagOpen;
    } else {
  ...
  parseScriptDataEndTagOpen(c) {
    if (c === 's') {
      return this.parseScriptDataEndTagNameS;
    } else {
      this.emit({ type: 'TEXT', content: '</' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
  ...
  parseScriptDataEndTagNameS(c) {
    if (c === 'c') {
      return this.parseScriptDataEndTagNameC;
    } else {
      this.emit({ type: 'TEXT', content: '</s' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    }
  }
  ...
  parseScriptDataEndTagNameC(c) {
    if (c === 'r') {
      return this.parseScriptDataEndTagNameR;
    } else {
      this.emit({ type: 'TEXT', content: '</sc' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    }
  }

  parseScriptDataEndTagNameR(c) {
    if (c === 'i') {
      return this.parseScriptDataEndTagNameI;
    } else {
      this.emit({ type: 'TEXT', content: '</scr' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    }
  }

  parseScriptDataEndTagNameI(c) {
    if (c === 'p') {
      return this.parseScriptDataEndTagNameP;
    } else {
      this.emit({ type: 'TEXT', content: '</scri' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    }
  }

  parseScriptDataEndTagNameP(c) {
    if (c === 't') {
      return this.parseScriptDataEndTag;
    } else {
      this.emit({ type: 'TEXT', content: '</scrip' });
      this.emit({ type: 'TEXT', content: c });
      return this.parseScriptData;
    }
  }

  parseScriptDataEndTag(c) {
    if (c == " ") {
      return this.parseScriptDataEndTag;
    } if (c == ">") {
      this.emit({ type: "END_TAG", tagName: "script" });
      return this.parseData;
    } else {
      this.emit({ type: "TEXT", content: "</script" });
      this.emit({ type: "TEXT", content: c });
      return this.parseScriptData;
    }
  }
